...
class minheap {
    private:
        ...
        void bubbleup(int index) {
            while(index != 0 && minheap[parent(index)]->freq > minheap[index]->freq) {
                swap(minheap[parent(index)], minheap[index]);
                index = parent(index);
            }
        }

        void bubbledown(int index) {
            while(minheap.size() > leftc(index)) {
                int smaller = leftc(index);
                if(minheap.size() != rightc(index)) {
                    if(minheap[leftc(index)]->freq > minheap[rightc(index)]->freq) smaller = rightc(index);
                }

                if(minheap[index]->freq < minheap[smaller]->freq) break;
                else {
                    swap(minheap[index], minheap[smaller]);
                }
                index = smaller;
            }
        }
